...
static void DispatchPreprocessors( Packet *p, tSfPolicyId policy_id, SnortPolicy *policy )
{
    ...
    do {
        ...
        // TODO delete this check for the profinet preprocesor and check how to create the right config for letting the proprocessor being called
		if (ppn->preproc_id == PP_PROFINET)
			ppn->func( p, ppn->context );
        else if ( preprocHandlesProto( p, ppn ) && IsPreprocessorEnabled( p, ppn->preproc_bit ) )
            ppn->func( p, ppn->context );
